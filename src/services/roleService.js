...
function ensureRoleRow(telegramId) {
    let row = db.prepare(`SELECT * FROM roles WHERE telegram_id = ?`).get(telegramId);

    if (!row) {
        db.prepare(`INSERT INTO roles (telegram_id, role) VALUES (?, 'student')`).run(telegramId);
        row = db.prepare(`SELECT * FROM roles WHERE telegram_id = ?`).get(telegramId);
    }
...
function setUserRole(telegramId, role) {
    ...
    db.prepare(`UPDATE roles SET role = ? WHERE telegram_id = ?`).run(role, telegramId);
}
...
function assignAdminToFaculty(telegramId, facultyId) {
    ...
    db.prepare(`INSERT OR IGNORE INTO admin_faculties (telegram_id, faculty_id) VALUES (?, ?)`).run(telegramId, facultyId);
}

function removeAdminFromFaculty(telegramId, facultyId) {
    db.prepare(`DELETE FROM admin_faculties WHERE telegram_id = ? AND faculty_id = ?`).run(telegramId, facultyId);

    const row = db.prepare(`SELECT COUNT(*) AS count FROM admin_faculties WHERE telegram_id = ?`).get(telegramId);
...
function getAdminFacultyIds(telegramId) {
    ...
    return db
        .prepare(`SELECT faculty_id FROM admin_faculties WHERE telegram_id = ? ORDER BY faculty_id`)
        .all(telegramId)
        .map(r => r.faculty_id);
}
...
function getAllAdmins() {
    return db.prepare(`
    SELECT r.telegram_id, r.role, af.faculty_id, f.name AS faculty_name
    FROM roles r
