...
async function handleChooseMyGroup(ctx) {
    ...
    if (!faculties.length) {
        return ctx.reply("❌ Hozircha fakultetlar kiritilmagan.");
    }

    const buttons = faculties.map((f) => [
        Markup.button.callback(`🏛 ${f.name}`, `selfac_${f.id}`)
    ]);
...
async function handleStudentSchedule(ctx) {
    ...
    const buttons = faculties.map((f) => [
        Markup.button.callback(`🏛 ${f.name}`, `fac_${f.id}`)
    ]);
...
async function handleTeacherScheduleStart(ctx) {
    await ctx.reply("👨‍🏫 *O‘qituvchi familiyasi yoki ismini yuboring:*", {
        parse_mode: "Markdown"
    });
}
...
async function handleTeacherSearchText(ctx) {
    ...
    const buttons = teachers.map((t) => [
        Markup.button.callback(`👨‍🏫 ${t.full_name}`, `teacher_${t.id}`)
    ]);
...
async function handleTodaySchedule(ctx) {
    ...
    if (user && user.group_id) {
        ...
        if (!rows.length) {
            return ctx.reply("📭 Bugun sizning guruhingiz uchun dars yo‘q.");
        }
        return ctx.reply(formatTodayGroupSchedule(rows), { parse_mode: "Markdown" });
    }

    return ctx.reply(
        "❌ Siz hali guruh tanlamagansiz.\nAvval `👥 Guruhingizni tanlang` tugmasini bosing.",
        { parse_mode: "Markdown" }
    );
}
...
async function handleWeeklySchedule(ctx) {
    ...
    if (user && user.group_id) {
        ...
        if (!rows.length) {
            return ctx.reply("📭 Sizning guruhingiz uchun haftalik jadval topilmadi.");
        }
...
async function handleMyGroup(ctx) {
    ...
    if (!user || !user.group_id) {
        return ctx.reply("❌ Siz hali guruh tanlamagansiz.", {
            ...Markup.inlineKeyboard([
                [Markup.button.callback("👥 Guruhni tanlash", "start_choose_group")]
            ])
        });
    }
    ...
    if (!info) {
        return ctx.reply("❌ Saqlangan guruh topilmadi.");
    }
...
async function handleHelp(ctx) {
    let text = `ℹ️ *Yordam*\n\n`;
    text += `👥 *Guruhingizni tanlang* — o‘zingiz o‘qiydigan guruhni saqlaydi.\n`;
    text += `🔁 *Guruhni almashtirish* — boshqa guruh tanlash imkonini beradi.\n`;
    text += `👤 *Mening guruhim* — saqlangan guruhni ko‘rsatadi.\n`;
    text += `📚 *Jadvalni ko‘rish* — istalgan guruh jadvalini ko‘rsatadi.\n`;
    text += `👨‍🏫 *O‘qituvchini qidirish* — o‘qituvchi jadvalini topadi.\n`;
    text += `📅 *Bugungi jadval* — saqlangan guruhning bugungi darslari.\n`;
    text += `🗓 *Haftalik jadval* — saqlangan guruhning haftalik jadvali.\n`;
...
async function handleCallback(ctx) {
    ...
    if (data === "go_home") {
        ...
        await ctx.reply("🏠 Bosh menyu", mainMenu(isAdmin));
        return;
    }
    ...
    if (data.startsWith("selfac_")) {
        ...
        const buttons = directions.map((d) => [
            Markup.button.callback(`📘 ${d.name}`, `seldir_${facultyId}_${d.id}`)
        ]);
    ...
    if (data.startsWith("seldir_")) {
        ...
        const buttons = courses.map((c) => [
            Markup.button.callback(`🎓 ${c.course}-kurs`, `selcourse_${facultyId}_${directionId}_${c.course}`)
        ]);
    ...
    if (data.startsWith("selcourse_")) {
        ...
        const buttons = groups.map((g) => [
            Markup.button.callback(`👥 ${g.group_name}`, `selgroup_${facultyId}_${directionId}_${course}_${g.id}`)
        ]);
    ...
    if (data.startsWith("fac_")) {
        ...
        const buttons = directions.map((d) => [
            Markup.button.callback(`📘 ${d.name}`, `dir_${facultyId}_${d.id}`)
        ]);
    ...
    if (data.startsWith("dir_")) {
        ...
        const buttons = courses.map((c) => [
            Markup.button.callback(`🎓 ${c.course}-kurs`, `course_${facultyId}_${directionId}_${c.course}`)
        ]);
    ...
    if (data.startsWith("course_")) {
        ...
        const buttons = groups.map((g) => [
            Markup.button.callback(`👥 ${g.group_name}`, `group_${facultyId}_${directionId}_${course}_${g.id}`)
        ]);
    ...
    if (data.startsWith("group_")) {
        ...
        await ctx.editMessageText(formatGroupSchedule(rows), {
            ...
            ...Markup.inlineKeyboard([
                [
                    Markup.button.callback("⬅️ Orqaga", `course_${facultyId}_${directionId}_${course}`),
                    Markup.button.callback("🏠 Bosh menyu", "go_home")
                ]
            ])
        });
